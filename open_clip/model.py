...
class CustomTextCLIP(nn.Module):
    ...
    @staticmethod
    def fgsm_attack(image, epsilon, data_grad, noise_range=(0.01, 0.04)):
        # Collect the element-wise sign of the data gradient
        sign_data_grad = data_grad.sign()
        # Generate random noise within the specified range
        min_noise, max_noise = noise_range
        noise = (max_noise - min_noise) * torch.rand(1) + min_noise
        print(f"fgsm attack!! with some noise {noise}")
        # Apply the random noise to the sign_data_grad
        sign_data_grad = sign_data_grad + noise

        # Create the perturbed image by adjusting it in the direction that increases the loss
    ...
    @staticmethod
    def apply_fgsm_defense(model, image, epsilon, target_class=0):
        # Set the model in evaluation mode
        model.eval()

        # Send the image to the same device as the model (e.g., GPU)
        image = image.to(model.device)
        
        # Create a copy of the image for gradient calculation
        ...
        # Calculate the loss
        loss = nn.CrossEntropyLoss()(output, torch.tensor([target_class]).to(model.device))
        ...
        # Call the FGSM attack to create a perturbed image
        perturbed_image = CustomTextCLIP.fgsm_attack(image, epsilon, data_grad)

        # Return the perturbed image
        return perturbed_image

    def encode_image(self, image, normalize: bool = False):
        self.apply_fgsm_defense(image)
        print('encoded image')
        features = self.visual(image)
        return F.normalize(features, dim=-1) if normalize else features
